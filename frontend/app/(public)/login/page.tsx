...
import { useAuth, PinRequiredError } from '@/hooks/useAuth'
...
function LoginForm() {
  ...
  const { login, unlockAfterLogin } = useAuth()
  ...
  // PIN step state (shown when login() detects a separate encryption PIN)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err instanceof PinRequiredError) {
        setPinData({
          keySalt: err.keySalt,
          encryptedKeys: err.encryptedKeys,
          verificationBlob: err.verificationBlob,
      ...
      if (err instanceof ApiError) {
        ...
        if ((err as any).code === 'EMAIL_NOT_VERIFIED') {
          const userEmail = (err as any).email || email
  ...
  const handlePinSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await unlockAfterLogin(
        pinValue,
        pinData.keySalt,
        pinData.encryptedKeys,
        pinData.verificationBlob,
        pinData.redirectTo
      )
    } catch (err) {
      setPinError((err as Error).message || 'PIN incorrecto')
    } finally {
      setIsLoading(false)
    }
  }
                ...
                <>
                  <div className="text-center mb-8">
                    ...
                    <p className="text-muted-foreground text-sm">
                      Tu cuenta tiene un PIN de cifrado separado. Introdúcelo para acceder a tus
                      datos.
                    </p>
                  </div>
                  ...
                  <form onSubmit={handlePinSubmit} className="space-y-5">
                    ...
                    <div className="space-y-2">
                      ...
                      <div
                        className={`relative transition-all duration-300 ${focusedField === 'pin' ? 'scale-[1.02]' : ''}`}
                      >
                        <div className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground">
                          <KeyRound
                            className={`w-4 h-4 transition-colors duration-300 ${focusedField === 'pin' ? 'text-blue-500' : ''}`}
                          />
                        </div>
                        <input
                          id="pin"
                  ...
                  <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div className="space-y-2">
                      ...
                      >
                        <div className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground">
                          <Mail
                            className={`w-4 h-4 transition-colors duration-300 ${focusedField === 'email' ? 'text-emerald-500' : ''}`}
                          />
                        </div>
                        <input
                          id="email"
